...
import { ebayService, EbayServiceError } from '../../services/commerce/EbayService';
...
function normalizeError(error: unknown): {
    ...
    if (error instanceof z.ZodError) {
        return {
            ...
            code: 'INVALID_EBAY_REQUEST',
        };
    }

    return {
        status: 500,
        message: error instanceof Error ? error.message : 'eBay inspect failed',
        code: 'EBAY_INSPECT_FAILED',
    };
}
...
export class EbayController {
    static async inspect(req: Request, res: Response) {
        ...
        try {
            const payload = EbayInspectSchema.parse(req.body);
            const data = await ebayService.inspect(payload);

            await logRequest({
                apiKeyId,
                url: `ebay-inspect://${payload.input}`,
                method: 'POST',
                statusCode: 200,
            ...
            await logRequest({
                apiKeyId,
                url: `ebay-inspect://${req.body?.input || 'unknown'}`,
                method: 'POST',
