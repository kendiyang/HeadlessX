...
const OPERATOR_SEEDS: OperatorSeed[] = [
    ...
    },
    {
        ...
        description:
            'Inspect subreddit listings, thread posts, and comments through Reddit JSON endpoints.',
        category: 'social',
        features: ['Threads', 'Comments', 'Subreddits', 'Metadata'],
        playgroundHref: '/playground/operators/reddit',
        apiBasePath: '/api/operators/reddit',
        order: 90,
    },
    {
        id: 'youtube',
    ...
    },
    {
        ...
        playgroundHref: '/playground/operators/amazon',
        apiBasePath: '/api/operators/amazon',
        order: 110,
    },
    {
        id: 'ebay',
        name: 'eBay',
        tagline: 'Marketplace listing intelligence',
        description:
            'Inspect listing metadata, seller details, pricing, and anti-bot diagnostics.',
        category: 'commerce',
        features: ['Listings', 'Pricing', 'Seller', 'Diagnostics'],
        playgroundHref: '/playground/operators/ebay',
        apiBasePath: '/api/operators/ebay',
        order: 112,
    },
    {
        id: 'walmart',
        name: 'Walmart',
        tagline: 'Retail catalog intelligence',
        description:
            'Inspect product metadata, pricing, seller details, and anti-bot diagnostics.',
        category: 'commerce',
        features: ['Products', 'Pricing', 'Seller', 'Diagnostics'],
        playgroundHref: '/playground/operators/walmart',
        apiBasePath: '/api/operators/walmart',
        order: 114,
    },
    {
        id: 'facebook',
...
class OperatorCatalogService {
    ...
    public async listOperators(): Promise<OperatorRecord[]> {
        ...
        const amazon = byId.get('amazon');
        const ebay = byId.get('ebay');
        const walmart = byId.get('walmart');
        ...
        if (amazon) {
            operators.push(this.buildActive(amazon));
        }

        if (ebay) {
            operators.push(this.buildActive(ebay));
        }

        if (walmart) {
            operators.push(this.buildActive(walmart));
        }

        if (reddit) {
            operators.push(this.buildActive(reddit));
        }
