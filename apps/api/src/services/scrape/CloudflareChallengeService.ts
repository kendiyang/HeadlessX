...
class CloudflareChallengeService {
    ...
    public async ensureBypassed(
        page: Page,
        options: {
            recheckAfterNavigation?: boolean;
            maxAttempts?: number;
            onAttempt?: (attempt: number, maxAttempts: number) => void;
        } = {}
    ): Promise<void> {
        let detection = await this.detectAfterNavigation(page, {
            recheck: options.recheckAfterNavigation,
        });
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            ...
            const bypassSuccess = await this.solveChallenge(page);
            ...
            await page.waitForTimeout(2000);
            detection = await this.detect(page);
