...
class ScraperService {
    ...
    private async ensureNoCloudflareChallenge(page: Page, jsEnabled = false) {
        await cloudflareChallengeService.ensureBypassed(page, {
            recheckAfterNavigation: jsEnabled,
            maxAttempts: 2,
        });
    }
    ...
    public async scrape(url: string, options: ScrapeOptions = {}): Promise<ScrapeResult> {
        ...
        try {
            ...
            page.on('response', response => {
                const isMainNavigation = response.request().isNavigationRequest()
            ...
            statusCode = navigationResponse?.status() || statusCode;
            ...
            const metadata = extractWebsiteMetadata(html, finalUrl);
