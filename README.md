...
### Dashboard Auth For Public Deployments

Before exposing the dashboard to the internet, enable dashboard session auth:

- `DASHBOARD_AUTH_ENABLED=true`
- `DASHBOARD_AUTH_SECRET` (32+ random characters)
- `DASHBOARD_AUTH_USERNAME`
- `DASHBOARD_AUTH_PASSWORD_HASH` (`scrypt:<salt-base64url>:<hash-base64url>`)
- `REDIS_URL` (used by distributed login rate limiting across instances)

Production mode rejects plaintext-only dashboard passwords by design.  
See [docs/setup-guide.md](docs/setup-guide.md) for the hash generation command and full env details.
