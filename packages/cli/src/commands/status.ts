...
export async function handleStatusCommand(options: StatusOptions): Promise<void> {
  ...
  if (apiKey && reachability.api.ok) {
    const requests = await Promise.allSettled([
      ...
      requestJson({ apiUrl: runtimeTargets.apiUrl, path: '/api/operators/amazon/status' }),
      requestJson({ apiUrl: runtimeTargets.apiUrl, path: '/api/operators/ebay/status' }),
      requestJson({ apiUrl: runtimeTargets.apiUrl, path: '/api/operators/walmart/status' }),
      requestJson({ apiUrl: runtimeTargets.apiUrl, path: '/api/operators/reddit/status' }),
    ...
    integrations = {
      ...
      amazon: requests[4].status === 'fulfilled' ? requests[4].value : { error: requests[4].reason?.message || 'failed' },
      ebay: requests[5].status === 'fulfilled' ? requests[5].value : { error: requests[5].reason?.message || 'failed' },
      walmart: requests[6].status === 'fulfilled' ? requests[6].value : { error: requests[6].reason?.message || 'failed' },
      reddit: requests[7].status === 'fulfilled' ? requests[7].value : { error: requests[7].reason?.message || 'failed' },
    };
    queue = requests[8].status === 'fulfilled' ? requests[8].value : { error: requests[8].reason?.message || 'failed' };
  } else if (apiKey) {
    integrations = {
