...
function resolveBooleanEnv(currentValue: string | undefined, fallback: boolean): string {
  const normalized = currentValue?.trim().toLowerCase();
  if (normalized && ['true', 'false', '1', '0', 'yes', 'no', 'on', 'off'].includes(normalized)) {
...
function resolveDashboardAuthEnv(
  current: Record<string, string | undefined>,
  options: { defaultEnabled: boolean }
): Record<string, string> {
  return {
    DASHBOARD_AUTH_ENABLED: resolveBooleanEnv(current.DASHBOARD_AUTH_ENABLED, options.defaultEnabled),
    DASHBOARD_AUTH_SECRET: resolveSecret(current.DASHBOARD_AUTH_SECRET),
    DASHBOARD_AUTH_USERNAME: current.DASHBOARD_AUTH_USERNAME?.trim() || 'admin',
    DASHBOARD_AUTH_PASSWORD_HASH: current.DASHBOARD_AUTH_PASSWORD_HASH?.trim() || '',
    DASHBOARD_AUTH_PASSWORD: current.DASHBOARD_AUTH_PASSWORD?.trim() || '',
  };
}

function syncSelfHostEnvFromCurrent(options: { defaultAuthEnabled: boolean } = { defaultAuthEnabled: false }): { apiUrl: string; webUrl: string } {
  ...
  upsertEnvValues(envPath, {
    ...
    ...resolveDashboardAuthEnv(current, { defaultEnabled: options.defaultAuthEnabled }),
  });
...
async function configureSelfHost(
  options: InitOptions,
  authOptions: { defaultAuthEnabled: boolean } = { defaultAuthEnabled: false }
  ...
  upsertEnvValues(envPath, {
    ...
    DASHBOARD_INTERNAL_API_KEY: resolveSecret(current.DASHBOARD_INTERNAL_API_KEY),
    CREDENTIAL_ENCRYPTION_KEY: resolveSecret(current.CREDENTIAL_ENCRYPTION_KEY),
    ...resolveDashboardAuthEnv(current, { defaultEnabled: authOptions.defaultAuthEnabled }),
  });
...
function syncDeveloperEnvFromCurrent(): { apiUrl: string; webUrl: string } {
  ...
  upsertEnvValues(envPath, {
    ...
    DASHBOARD_INTERNAL_API_KEY: resolveSecret(current.DASHBOARD_INTERNAL_API_KEY),
    CREDENTIAL_ENCRYPTION_KEY: resolveSecret(current.CREDENTIAL_ENCRYPTION_KEY),
    ...resolveDashboardAuthEnv(current, { defaultEnabled: false }),
  });
...
function syncProductionEnvFromCurrent(options: InitOptions): { apiUrl: string; webUrl: string } {
  syncSelfHostEnvFromCurrent({ defaultAuthEnabled: true });
...
async function configureProduction(options: InitOptions): Promise<{ apiUrl: string; webUrl: string }> {
  const urls = await configureSelfHost(options, { defaultAuthEnabled: true });
