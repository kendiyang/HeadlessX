...
| `/api/operators/amazon/*` | No mandatory external API key, but results depend on Amazon anti-bot posture and browser session trust |
| `/api/operators/reddit/*` | No mandatory external API key, but Reddit can rate-limit repeated unauthenticated requests |
...
| Method | Path | Purpose | Notes |
| --- | --- | --- | --- |
| `POST` | `/api/operators/reddit/inspect` | Inspect Reddit thread or listing metadata using `.json` endpoints | Accepts Reddit URL/path, `redd.it` short URL, or post ID |
| `GET` | `/api/operators/reddit/status` | Reddit operator status | Lightweight availability check |

Reddit inspect request fields:

| Field | Type | Meaning |
| --- | --- | --- |
| `input` | `string` | Reddit URL/path, `redd.it` short URL, or post ID |
| `sort` | `string` | Optional sort mode such as `confidence`, `new`, `top`, `hot`, `rising` |
| `timeframe` | `hour \| day \| week \| month \| year \| all` | Optional timeframe filter |
| `limit` | `number` | Max records to include (`1-500`) |
| `depth` | `number` | Max nested comment depth for thread extraction (`0-10`) |
| `timeout` | `number` | Request timeout in milliseconds |
| `includeRaw` | `boolean` | Include raw Reddit JSON payload for debugging |
