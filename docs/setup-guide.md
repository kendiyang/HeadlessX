...
- that includes values such as `YT_ENGINE_URL`, `INTERNAL_API_URL`, `DASHBOARD_INTERNAL_API_KEY`, and dashboard auth settings
...
- `DASHBOARD_AUTH_PASSWORD_HASH` (recommended)
- `REDIS_URL`

Generate `DASHBOARD_AUTH_PASSWORD_HASH` with:

```bash
node -e "const { randomBytes, scryptSync } = require('crypto'); const password='ChangeMe!'; const salt=randomBytes(16); const hash=scryptSync(password, salt, 64, {N:16384,r:8,p:1,maxmem:64*1024*1024}); const b=v=>v.toString('base64url'); console.log(`scrypt:${b(salt)}:${b(hash)}`);"
```

Note:

- prefer `scrypt:<salt>:<hash>` format in dotenv files
- if using legacy `scrypt$<salt>$<hash>`, escape as `scrypt\$<salt>\$<hash>`

Security notes for production:

- keep `DASHBOARD_AUTH_ENABLED=true`
- do not use plaintext `DASHBOARD_AUTH_PASSWORD` in production
- keep `DASHBOARD_AUTH_SECRET` at 32+ characters from a secure random source
- set `DASHBOARD_AUTH_COOKIE_DOMAIN` when dashboard runs on a dedicated domain (for example `dashboard.example.com`)
- keep `DASHBOARD_AUTH_RATE_LIMIT_FAIL_OPEN=false` (or unset) to fail closed when Redis is unavailable
...
## Security And Monitoring Runbook

### 1) Rotate Exposed API Key Immediately

Create a new key and revoke the old exposed key in one command:

```bash
API_URL="http://localhost:38473"
API_KEY="current_admin_or_internal_key"
pnpm run security:rotate-api-key -- \
  --api-url "$API_URL" \
  --admin-api-key "$API_KEY" \
  --old-key "$API_KEY" \
  --new-name "rotated-after-exposure-$(date +%Y%m%d-%H%M%S)"
```

Optional: write the new key into `.env` as `HX_API_KEY`:

```bash
pnpm run security:rotate-api-key -- \
  --api-url "http://localhost:38473" \
  --admin-api-key "current_admin_or_internal_key" \
  --old-key "old_exposed_key" \
  --env-file .env \
  --env-var HX_API_KEY
```

### 2) Reddit Production SLO Alert Check

Run rolling SLO checks for Reddit inspect traffic:
...
- `0`: SLO healthy
- `2`: SLO breached (alert condition)
- `1`: script/runtime failure
